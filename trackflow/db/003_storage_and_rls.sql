...
-- 1) profiles: allow users to select their own profile and allow admins to manage
...
-- 2) shipments: customers can create/select their shipments; drivers can select assigned shipments and update status; dispatchers and admins have broader access
...
-- Drivers: can update status fields for shipments assigned to them (allow only status changes to permitted values)
...
-- Dispatchers: may assign drivers (update driver_id) and have broader select access
...
create policy if not exists shipments_dispatcher_update on public.shipments
  for update
  using ( exists (select 1 from public.profiles p where p.id = auth.uid and p.role = 'dispatcher') )
  with check ( true ); -- consider restricting which fields can be changed via check expressions


-- 3) drivers table: allow drivers to update their own location/status; dispatchers/admins to view/manage
...
-- optionally allow dispatchers to update driver fields (assignment, status)
...
create policy if not exists storage_pod_allow_upload on storage.objects
  for insert
  using ( bucket_id = 'pod' and auth.role() = 'authenticated' )
  with check ( bucket_id = 'pod' and auth.role() = 'authenticated' );
...
create policy if not exists storage_pod_allow_select_uploader on storage.objects
  for select
  using (
    bucket_id = 'pod' and (
...
-- 3) Adjust policy granularity as needed: the "with check ( true )" used for some dispatcher/admin policies can be tightened to restrict fields.
